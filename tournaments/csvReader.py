...
class DjangoSimpleFetcher:
    ...
    def get_or_create_person(
        first_name, last_name, gender=Person.UNKNOWN, nationality=None, born=None
    ):
        try:
            ...
            if nationality and result.nationality is None:
                ...
                Person.objects.get(first_name=first_name, last_name=last_name).update(
    ...
    def create_multigame(tournament, phase, local, visitor, l_score, v_score):
        result = MultiGame.objects.get_or_create(
            tournament=tournament, phase=phase, local=local, visitor=visitor
        )
    ...
    def get_or_create_game_phase(category, round, number, create):
        ...
        if (
            get_round.encode("utf-8") == b"\xc2\xbc"
            or get_round.encode("utf-8") == b"\xc2\xbd"
            or get_round == "\xc2\xbc"
        ):
            get_round = "1/4"
        if create:
        ...
        if scores and int(scores) > 0:
            ...
            try:
                ...
                result = PlayerStadistic.objects.get_or_create(
                    game=game, player=player, points=scores
                )
            return result
        else:
            return None, False
    ...
    def assign_club_to_person(person_club):
        person = Person.objects.get(
            first_name=person_club.first_name, last_name=person_club.last_name
        )
...
def check_team_players(team, person1, person2):
    ...
    if players[0].first_name in ["Bye", "bye"] or players[1].first_name in [
        "Bye",
        "bye",
    ]:
        return True
...
class DjangoCsvFetcher:
    ...
    def create_csv_phase(csv_game, create):
        ...
        if create:
            result, created = GameRound.objects.get_or_create(
                category=csv_game.category, round=round, number_teams=csv_game.nteams
            )
        else:
            result, created = (
    ...
    def create_padel_csv_game(game):
        ...
        # create local team
        local_team, created = create_or_fetch_team2(
            persons[0],
            persons[1],
            game.local,
            game.division,
        ...
        if game.is_multigame():
            ...
            DjangoSimpleFetcher.get_or_create_player(
                persons[0], sublocal_team, None, tournament.id
            )
            DjangoSimpleFetcher.get_or_create_player(
                persons[1], sublocal_team, None, tournament.id
            )

        # create visitor team
        visitor_team, created = create_or_fetch_team2(
            persons[2],
            persons[3],
            game.visitor,
            game.division,
            game.is_pair,
            game.visitor_country,
            game.is_nations(),
            game.is_clubs(),
        )
        ...
        if game.is_multigame():
            ...
            DjangoSimpleFetcher.get_or_create_player(
                persons[2], subvisitor_team, None, tournament.id
            )
            DjangoSimpleFetcher.get_or_create_player(
                persons[3], subvisitor_team, None, tournament.id
            )
    ...
    def create_touch_csv_game(game):
        ...
        tournament, created = DjangoSimpleFetcher.get_or_create_tournament(
            game.federation, game.tournament_name, game.division, type
        )
    ...
    def create_csv_fit_statistic(csv_stats):
        ...
        tournament, created = DjangoSimpleFetcher.get_or_create_tournament(
            "", csv_stats.tournament_name, csv_stats.division, "TOUCH"
        )
        DjangoSimpleFetcher.print_fetch_result(tournament, created)

        team, created = DjangoSimpleFetcher.get_or_create_team(
            csv_stats.team, csv_stats.division
        )
        DjangoSimpleFetcher.print_fetch_result(team, created)

        person, created = DjangoSimpleFetcher.get_or_create_person(
            csv_stats.first_name, csv_stats.last_name, csv_stats.gender
        )
        DjangoSimpleFetcher.print_fetch_result(person, created)

        player, created = DjangoSimpleFetcher.get_or_create_player(
            person, team, csv_stats.number, tournament
        )
    ...
    def create_csv_nts_player_statistic(csv_stats):
        ...
        if csv_stats.visitor_score:  # if true nts stadistic otherwise player insert.
            ...
            game = DjangoSimpleFetcher.get_game(
                tournament,
                phase,
                local_team,
                csv_stats.local_score,
                visitor_team,
                csv_stats.visitor_score,
                False,
            )
            ...
            print(
                "GameStadistic skipped: there are no tries for player: {:s}\n ".format(
                    str(player)
                )
            )
...
class CsvReader:
    ...
    def create_django_object(self, csv_object):
        if self._type == self.PHASE and isinstance(csv_object, csvdata.CsvPhase):
        ...
        elif self._type == self.NTS_STATISTIC and isinstance(
            ...
            DjangoSimpleFetcher.get_or_create_person(
                csv_object.first_name,
                csv_object.last_name,
                csv_object.gender,
