...
def player_directory_path(instance, filename):
    return "player_media/" + normalize(
        no_german_chars(
            "{0}-{1}-{2}-{3}".format(
                instance.last_name, instance.first_name, instance.email, filename
            )
        )
    )
...
class Registration(models.Model):
    ...
    player_a = models.ForeignKey(
        PadelPerson, related_name="player_a", on_delete=models.DO_NOTHING
    )
    player_b = models.ForeignKey(
        PadelPerson, related_name="player_b", on_delete=models.DO_NOTHING
    )
